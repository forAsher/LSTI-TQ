...
class ParameterPool(Enum):
    RANDOM = Parameter(stages=[1, 10], cores=[[1, 1], [1, 1]], train_steps=[40000, 5000],
                       batch_sizes=[100, 200], learning_rates=[0.0001, 0.001], keep_ratios=[1.0, 1.0])
...
class AbstractNN:
    ...
    def predict(self, input_key):
        ...
        return float(tmp_res[0][0])


# Netural Network Model
class TrainedNN:
    def __init__(self,j, threshold, useThreshold, cores, train_step_num, batch_size, learning_rate, keep_ratio, train_x,
                 train_y,
                 test_x, test_y):
        # set parameters
        if cores is None:
        ...
        self.train_step_nums = train_step_num+j*5000
        if self.train_step_nums > max_train_steps:
            self.train_step_nums = max_train_steps
    ...
    # train model
    def train(self):
        for i in range(len(self.core_nums) - 1):  # 执行多少次就是有多少个隐藏层
            # 定义隐藏层输出张量h_fc[i]，使用tf.nn.relu激活函数，tf.matmul(x,w1)+b1表示神经元的连接
